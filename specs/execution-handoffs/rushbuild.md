...
`RUSHB-001` defines a reviewable Rust execution handoff using a source-preserving shell runner.

The goal is to make Rust build and execution behavior visible, versioned, and reviewable before it runs locally or in CI.
...
- which Rust crate or source tree is being handed off
...
- which Cargo build command runs
...
- Rust source and asset files
- optional ignore rules
- output runner path

The reference implementation runs `cargo vendor` before packing so vendored dependencies can travel with the source payload.
...
It helps make Rust execution handoff easier to inspect before local or CI execution.
